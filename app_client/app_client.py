...
def sendSMS(messageBody, to):
    ...
    for n in numbers:
       try:
           ...
           sid = message.sid
           logger.debug("Sent sms: %s", bid, str(n))
       except Exception as ex:
        ...
        if msg["body"] == "connected":
            ...
            try:
                ...
                if bid not in config["bridges"]:
                    logger.info("button_client. Message received from unregistered bridge: %s", bid)
                    return
                found = False
                rx_n = 0
                for body in msg["body"]:
                    ...
                    if "n" in body:
                        if rx_n <= body["n"]:
                            ...
                            found = True
                            del body["n"]
                    if "a" not in body:  # "a" is a message ack
                        logger.info("onMessage to process: %s", str(json.dumps(body, indent=4)))
                ...
                logger.warning("onmessage. Problem processing message, type: %s, exception: %s", str(type(ex)), str(ex.args))
