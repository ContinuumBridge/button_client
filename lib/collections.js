...
var userAccessAllowed = function(loggedInUserId, user) {

    var loggedInUser = Users.findOne(loggedInUserId);
    // Located in methods.js
    return userCanModifyUser(loggedInUser, user);
}

Users.allow({
  'insert': function(userId, doc) {
      return userAccessAllowed(userId, doc);
  },
...
organisationAccessAllowed = function(userId, organisationId) {

    //var user = Users.build(Users.findOne(userId));
    var loggedInUser = Users.findOne(userId);
    if (!loggedInUser)
        throw new Meteor.Error('Unauthorized: User not logged in');

    if (!(loggedInUser.organisationIds && loggedInUser.organisationIds[0]))
        throw new Meteor.Error('Unauthorized: User is not in any organisations');
    
    if (organisationId 
        && _.contains(loggedInUser.organisationIds, organisationId)
        && !Roles.userIsInRole(userId, ['readOnly'])) {
...
listAccessAllowed = function(userId, list) {
...
buttonAccessAllowed = function(userId, button) {
...
Models.Screenset = Graviton.Model.extend({
    ...
    updateLEDs: function(nodeId, colour, remove) {
        
        var leds = this.get('leds') || {};
        console.log('updateLEDs leds 1', leds);
        if (remove) {
            delete leds[nodeId];
        } else {
            leds[nodeId] = colour;
        }
        console.log('updateLEDs leds 2', leds);
        console.log('updateLEDs this._id', this._id);
        Screensets.update(this._id, {
            $set: { leds: leds }
        });
    }
});
...
screensetAccessAllowed = function(userId, screenset) {
...
// Nodes
Models.Node = Graviton.Model.extend({
    belongsTo: {
        screenset: {
          collection: 'screensets',
          field: 'screensetId'
...
Nodes = Graviton.define('nodes', {
    modelCls: Models.Node
});

nodeAccessAllowed = function(userId, node) {

    var screensetId = node.get('screensetId');
...
Nodes.allow({
    'insert': function(userId, doc) {
        return nodeAccessAllowed(userId, doc);
    },
...
// NodeConnections
Models.NodeConnection = Graviton.Model.extend({
    belongsTo: {
        source: {
            collection: 'nodes',
            field: 'sourceId'
...
NodeConnections = Graviton.define('nodeConnections', {
    modelCls: Models.NodeConnection
});

var nodeConnectionAccessAllowed = function(userId, connection) {
...
NodeConnections.allow({
    'insert': function(userId, doc) {
        return nodeConnectionAccessAllowed(userId, doc);
    },
